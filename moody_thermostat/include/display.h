...
uint16_t  bg_color =0;
uint16_t  last_display_millis = 0;
...
// =======================================================================================
void plotNeedle(int16_t temperature, uint16_t ms_delay)
{
  int16_t angle = temperature * 2;
...
void displayUpdate(int temperature)
{
  if (millis() - last_display_millis < 1000) { 
    return;   
  }
  last_display_millis = millis();
 plotNeedle(temperature, 0);
}